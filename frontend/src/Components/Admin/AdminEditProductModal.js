...
import { adminUpdateProduct, fetchOptions } from "../../services/api";
...
const AdminEditProductModal = ({ product, isOpen, onClose, categories }) => {
  ...
  useEffect(() => {
    const loadOptions = async () => {
      try {
        ...
        setOptionsData(response.data.categoryOptions);
        setShowCategories(
          response.data.categoryOptions.categories.map((cat) => cat.name)
        );
      } catch (err) {
  ...
  useEffect(() => {
    if (product) {
      ...
      setAdditionalInfoSections(
        product?.additionalInfo || [{ id: Date.now(), info: "", images: [] }]
      );
  ...
  const handleAdditionalImageChange = (id, files) => {
    const updatedSections = additionalInfoSections.map((section) =>
      section.id === id
        ? {
            ...section,
            images: [...(section.images || []), ...Array.from(files)],
          }
        : section
    );
    setAdditionalInfoSections(updatedSections);
  ...
  const removeAdditionalInfoSection = (id) => {
    const filteredSections = additionalInfoSections?.filter(
  ...
  return (
    ...
    >
      <motion.div
        ...
        animate={{ scale: 1 }}
        exit={{ scale: 0.8 }}
        className="bg-gray-900 text-white w-full max-w-4xl mx-auto p-6 rounded-lg shadow-lg relative max-h-[90vh] overflow-y-auto"
      >
        ...
        <form onSubmit={handleSubmit(onSubmit)} encType="multipart/form-data">
          <div className="flex">
            ...
            <div className="w-1/2 pr-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    value={updatedProduct?.categoryName || ""} // Ensure category is selected correctly
            ...
            <div className="w-1/2 pl-4">
              ...
              {updatedProduct?.category && (
                <div className="mt-6">
                  {updatedProduct?.category && optionsData?.categories && (
                    <div className="mt-6">
                      ...
                      {optionsData.categories
                        ...
                        ?.fields?.map((field, index) => {
                          ...
                          const selectOptions =
                            optionsData.categorySelectOptions?.[field];
                          ...
                          return (
                            <div key={index} className="mb-3">
                              ...
                              Array.isArray(selectOptions) ? (
                                ...
                                (selectOptions || ["Yes"]).map((option, i) => (
                                  <label
                                    key={i}
            ...
            {additionalInfoSections?.map((section) => (
              ...
              >
                ...
                <input
                  ...
                  multiple
                  onChange={(e) =>
                    handleAdditionalImageChange(section.id, e.target.files)
                  }
                  className="w-full mt-2 p-2 bg-gray-700 rounded-md"
                />
                <div className="grid grid-cols-2 gap-2 mt-2">
