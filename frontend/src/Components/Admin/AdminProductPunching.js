...
const AdminProductPunching = () => {
  ...
  const [categoryFieldTypes, setCategoryFieldTypes] = useState({});
  ...
  useEffect(() => {
    const fetchCategoryOptions = async () => {
      try {
        ...
        setCategories(response.data.categoryOptions.categories);
        setFieldTypes(response.data.categoryOptions.fieldTypes);
        setCategoryFieldTypes(response.data.categoryOptions.categoryFieldTypes);
        setCategorySelectOptions(
          response.data.categoryOptions.categorySelectOptions
        );
      } catch (error) {
  ...
  const handleCategoryChange = (e) => {
    ...
    const selectedCategoryObj = categories.find((cat) => cat.name === selected);
    if (selectedCategoryObj) {
      const extraFields = {};
      selectedCategoryObj.fields.forEach((field) => {
        extraFields[field] = "";
      });
  ...
  const handleCategoryDetailChange = (e, key) => {
    if (fieldTypes[key] === "multi-select") {
      setCategoryDetails({
        ...categoryDetails,
    ...
    setAdditionalInfo((prevInfo) =>
      prevInfo.map((info, i) =>
        i === infoIndex
          ? {
              ...info,
              images: info.images.filter((_, idx) => idx !== imageIndex),
            }
          : info
      )
    );
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
              ...
              {/*<div>
                ...
                {/*<div className="mt-2 flex flex-wrap">
                  {colors.map((color, index) => (
                    <span
                      key={index}
                      className="flex items-center px-3 py-1 rounded-full mr-2"
                          ...
                          categorySelectOptions[key]?.map((option) => (
                            ...
                            >
                              <input
                                ...
                                id={`${key}-${option}`}
                                checked={
                                  categoryDetails[key]?.includes(option) ||
                                  false
                                }
                                onChange={(e) => {
                              ...
                              />
                              <label
                                className="form-check-label text-white fw-bold ms-1"
                                htmlFor={`${key}-${option}`}
                              >
                                {option}
                              </label>
          ...
          <div>
            ...
            {additionalInfo.map((info, index) => (
              <div
                key={index}
                className="mb-4 border p-4 rounded-lg bg-gray-700"
              >
                <button
